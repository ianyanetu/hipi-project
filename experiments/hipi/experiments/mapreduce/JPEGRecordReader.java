...
public class JPEGRecordReader extends
	...
	static private boolean _singleton_emit;
	static byte[] _byte_array_data;
	public static JPEGImageUtil _decoder;
	...
	public ImageHeader getCurrentKey() throws IOException, InterruptedException {
		...
		_decoder = JPEGImageUtil.getInstance();
		/*
		CodecManager.getDecoder(ImageType.JPEG_IMAGE);
		InputStream istream = _decoder.getInputStream();
		
		ImageHeader header = _decoder.decodeImageHeader(istream);
		istream.close();
		*/
		//_decoder = CodecManager.getDecoder(ImageType.JPEG_IMAGE);
		InputStream input = new ByteArrayInputStream(_byte_array_data);
		ImageHeader header = _decoder.decodeImageHeader(input);
		input.close();
	...
	public FloatImage getCurrentValue() throws IOException,
		...
		_decoder = JPEGImageUtil.getInstance();
		InputStream input = new ByteArrayInputStream(_byte_array_data);
		FloatImage image = _decoder.decodeImage(input);
		input.close();
